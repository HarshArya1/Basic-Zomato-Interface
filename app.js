const restaurants=[
    {
      "image": "second",
      "name": "Tandoor Treat",
      "rating": "3.0",
      "food_type": "Greek",
      "Price_for_two": 731,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "8.3",
      "offers": 26,
      "alcholol": true,
      "Restaurant_open_time": 14,
      "Restaurant_close_time": 2
    },
    {
      "image": "sixth",
      "name": "Naan Nirvana",
      "rating": "4.9",
      "food_type": "Greek",
      "Price_for_two": 1707,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "6.6",
      "offers": 9,
      "alcholol": false,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "fourth",
      "name": "Indigo Kitchen",
      "rating": "4.3",
      "food_type": "Mediterranean",
      "Price_for_two": 922,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "7.3",
      "offers": 25,
      "alcholol": false,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "fourth",
      "name": "Spice Garden",
      "rating": "3.1",
      "food_type": "Indian",
      "Price_for_two": 1521,
      "location": "Hanuman Mandir",
      "Distance_from_Customer_house": "9.7",
      "offers": 31,
      "alcholol": false,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "third",
      "name": "Golden Thali",
      "rating": "4.3",
      "food_type": "French",
      "Price_for_two": 1392,
      "location": "Shivaji Stand",
      "Distance_from_Customer_house": "5.4",
      "offers": 25,
      "alcholol": false,
      "Restaurant_open_time": 5,
      "Restaurant_close_time": 17
    },
    {
      "image": "fifth",
      "name": "Curry & Co.",
      "rating": "4.2",
      "food_type": "Japanese",
      "Price_for_two": 2177,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "8.5",
      "offers": 16,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "third",
      "name": "Naan Nirvana",
      "rating": "4.1",
      "food_type": "Chinese",
      "Price_for_two": 1298,
      "location": "Hanuman Mandir",
      "Distance_from_Customer_house": "2.9",
      "offers": 21,
      "alcholol": true,
      "Restaurant_open_time": 21,
      "Restaurant_close_time": 9
    },
    {
      "image": "First",
      "name": "Golden Thali",
      "rating": "3.8",
      "food_type": "Greek",
      "Price_for_two": 1441,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "2.4",
      "offers": 27,
      "alcholol": true,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "third",
      "name": "Saffron Spice",
      "rating": "4.8",
      "food_type": "French",
      "Price_for_two": 2190,
      "location": "Red Fort",
      "Distance_from_Customer_house": "9.1",
      "offers": 33,
      "alcholol": true,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "nine",
      "name": "The Curry House",
      "rating": "3.7",
      "food_type": "Indian",
      "Price_for_two": 1480,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "1.6",
      "offers": 23,
      "alcholol": true,
      "Restaurant_open_time": 14,
      "Restaurant_close_time": 2
    },
    {
      "image": "tenth",
      "name": "Bombay Bites",
      "rating": "4.5",
      "food_type": "Thai",
      "Price_for_two": 721,
      "location": "Red Fort",
      "Distance_from_Customer_house": "7.8",
      "offers": 33,
      "alcholol": false,
      "Restaurant_open_time": 21,
      "Restaurant_close_time": 9
    },
    {
      "image": "fifth",
      "name": "Desi Vibes",
      "rating": "4.3",
      "food_type": "Greek",
      "Price_for_two": 2253,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "6.2",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "second",
      "name": "Taste of Punjab",
      "rating": "4.5",
      "food_type": "Thai",
      "Price_for_two": 1924,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "10.7",
      "offers": 14,
      "alcholol": false,
      "Restaurant_open_time": 4,
      "Restaurant_close_time": 16
    },
    {
      "image": "sixth",
      "name": "Indigo Kitchen",
      "rating": "3.4",
      "food_type": "Greek",
      "Price_for_two": 2027,
      "location": "Red Fort",
      "Distance_from_Customer_house": "5.9",
      "offers": 28,
      "alcholol": false,
      "Restaurant_open_time": 4,
      "Restaurant_close_time": 16
    },
    {
      "image": "First",
      "name": "Flavors of India",
      "rating": "4.1",
      "food_type": "Thai",
      "Price_for_two": 2048,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "3.6",
      "offers": 8,
      "alcholol": true,
      "Restaurant_open_time": 0,
      "Restaurant_close_time": 12
    },
    {
      "image": "fourth",
      "name": "Naan Nirvana",
      "rating": "4.6",
      "food_type": "French",
      "Price_for_two": 687,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "1.8",
      "offers": 20,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "second",
      "name": "Bombay Bites",
      "rating": "5.0",
      "food_type": "French",
      "Price_for_two": 1007,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "6.6",
      "offers": 23,
      "alcholol": false,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "First",
      "name": "Tandoori Tales",
      "rating": "3.2",
      "food_type": "Greek",
      "Price_for_two": 527,
      "location": "Gauri Temple",
      "Distance_from_Customer_house": "3.0",
      "offers": 13,
      "alcholol": true,
      "Restaurant_open_time": 21,
      "Restaurant_close_time": 9
    },
    {
      "image": "sixth",
      "name": "Bombay Bites",
      "rating": "4.6",
      "food_type": "Mediterranean",
      "Price_for_two": 2011,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "10.6",
      "offers": 34,
      "alcholol": true,
      "Restaurant_open_time": 12,
      "Restaurant_close_time": 0
    },
    {
      "image": "Eight",
      "name": "Curry & Co.",
      "rating": "3.1",
      "food_type": "Thai",
      "Price_for_two": 2254,
      "location": "Humayun's Dad",
      "Distance_from_Customer_house": "10.4",
      "offers": 15,
      "alcholol": true,
      "Restaurant_open_time": 3,
      "Restaurant_close_time": 15
    },
    {
      "image": "sixth",
      "name": "Taste of Punjab",
      "rating": "4.5",
      "food_type": "Greek",
      "Price_for_two": 1148,
      "location": "Great Park",
      "Distance_from_Customer_house": "5.0",
      "offers": 5,
      "alcholol": false,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "sixth",
      "name": "The Curry House",
      "rating": "3.9",
      "food_type": "Chinese",
      "Price_for_two": 1140,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "5.3",
      "offers": 34,
      "alcholol": true,
      "Restaurant_open_time": 1,
      "Restaurant_close_time": 13
    },
    {
      "image": "third",
      "name": "Spicy Sizzle",
      "rating": "3.7",
      "food_type": "Greek",
      "Price_for_two": 1269,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "5.5",
      "offers": 33,
      "alcholol": true,
      "Restaurant_open_time": 23,
      "Restaurant_close_time": 11
    },
    {
      "image": "Eight",
      "name": "Royal Rasoi",
      "rating": "4.1",
      "food_type": "French",
      "Price_for_two": 1215,
      "location": "India Gate",
      "Distance_from_Customer_house": "10.7",
      "offers": 19,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "seventh",
      "name": "Royal Rasoi",
      "rating": "4.2",
      "food_type": "Thai",
      "Price_for_two": 743,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "5.9",
      "offers": 29,
      "alcholol": false,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "sixth",
      "name": "Indigo Kitchen",
      "rating": "3.0",
      "food_type": "Italian",
      "Price_for_two": 564,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "4.6",
      "offers": 19,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "First",
      "name": "Bombay Bites",
      "rating": "4.3",
      "food_type": "Chinese",
      "Price_for_two": 2300,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "7.4",
      "offers": 21,
      "alcholol": true,
      "Restaurant_open_time": 4,
      "Restaurant_close_time": 16
    },
    {
      "image": "fourth",
      "name": "Indigo Kitchen",
      "rating": "4.5",
      "food_type": "Greek",
      "Price_for_two": 2228,
      "location": "Red Fort",
      "Distance_from_Customer_house": "9.2",
      "offers": 9,
      "alcholol": true,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "First",
      "name": "Spice Garden",
      "rating": "4.6",
      "food_type": "Italian",
      "Price_for_two": 1677,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "4.7",
      "offers": 17,
      "alcholol": true,
      "Restaurant_open_time": 4,
      "Restaurant_close_time": 16
    },
    {
      "image": "fourth",
      "name": "Royal Rasoi",
      "rating": "4.3",
      "food_type": "French",
      "Price_for_two": 1364,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "9.3",
      "offers": 26,
      "alcholol": true,
      "Restaurant_open_time": 8,
      "Restaurant_close_time": 20
    },
    {
      "image": "sixth",
      "name": "Spicy Sizzle",
      "rating": "3.1",
      "food_type": "Indian",
      "Price_for_two": 716,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "1.5",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 19,
      "Restaurant_close_time": 7
    },
    {
      "image": "First",
      "name": "Indigo Kitchen",
      "rating": "3.9",
      "food_type": "Indian",
      "Price_for_two": 848,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "4.5",
      "offers": 5,
      "alcholol": false,
      "Restaurant_open_time": 17,
      "Restaurant_close_time": 5
    },
    {
      "image": "fourth",
      "name": "Biryani Bistro",
      "rating": "3.6",
      "food_type": "Greek",
      "Price_for_two": 2329,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "4.9",
      "offers": 7,
      "alcholol": true,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "Eight",
      "name": "Naan Nirvana",
      "rating": "4.4",
      "food_type": "Greek",
      "Price_for_two": 1359,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "4.2",
      "offers": 27,
      "alcholol": true,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "fifth",
      "name": "Curry & Co.",
      "rating": "4.7",
      "food_type": "American",
      "Price_for_two": 1938,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "2.5",
      "offers": 26,
      "alcholol": true,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "fourth",
      "name": "Chaat Corner",
      "rating": "4.0",
      "food_type": "Indian",
      "Price_for_two": 1127,
      "location": "Red Fort",
      "Distance_from_Customer_house": "4.0",
      "offers": 18,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "third",
      "name": "Spice Garden",
      "rating": "3.5",
      "food_type": "Mexican",
      "Price_for_two": 975,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "3.0",
      "offers": 31,
      "alcholol": true,
      "Restaurant_open_time": 7,
      "Restaurant_close_time": 19
    },
    {
      "image": "fifth",
      "name": "Saffron Spice",
      "rating": "3.4",
      "food_type": "Mexican",
      "Price_for_two": 974,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "4.2",
      "offers": 17,
      "alcholol": false,
      "Restaurant_open_time": 8,
      "Restaurant_close_time": 20
    },
    {
      "image": "fourth",
      "name": "Bombay Bites",
      "rating": "4.1",
      "food_type": "Mediterranean",
      "Price_for_two": 2405,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "5.1",
      "offers": 8,
      "alcholol": false,
      "Restaurant_open_time": 3,
      "Restaurant_close_time": 15
    },
    {
      "image": "fifth",
      "name": "Chaat Corner",
      "rating": "4.7",
      "food_type": "American",
      "Price_for_two": 2295,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "3.6",
      "offers": 25,
      "alcholol": false,
      "Restaurant_open_time": 5,
      "Restaurant_close_time": 17
    },
    {
      "image": "second",
      "name": "Flavors of India",
      "rating": "4.9",
      "food_type": "Greek",
      "Price_for_two": 1312,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "2.3",
      "offers": 28,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "Eight",
      "name": "Spicy Sizzle",
      "rating": "3.6",
      "food_type": "Japanese",
      "Price_for_two": 2421,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "3.7",
      "offers": 22,
      "alcholol": false,
      "Restaurant_open_time": 7,
      "Restaurant_close_time": 19
    },
    {
      "image": "tenth",
      "name": "Indigo Kitchen",
      "rating": "3.7",
      "food_type": "Thai",
      "Price_for_two": 644,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "1.0",
      "offers": 14,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "Eight",
      "name": "Royal Rasoi",
      "rating": "4.4",
      "food_type": "Japanese",
      "Price_for_two": 1517,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "4.1",
      "offers": 29,
      "alcholol": true,
      "Restaurant_open_time": 7,
      "Restaurant_close_time": 19
    },
    {
      "image": "sixth",
      "name": "Chaat Corner",
      "rating": "4.4",
      "food_type": "Greek",
      "Price_for_two": 946,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "8.7",
      "offers": 7,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "fourth",
      "name": "Taste of Punjab",
      "rating": "3.8",
      "food_type": "Japanese",
      "Price_for_two": 2321,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "6.2",
      "offers": 23,
      "alcholol": true,
      "Restaurant_open_time": 20,
      "Restaurant_close_time": 8
    },
    {
      "image": "fifth",
      "name": "Kebab King",
      "rating": "4.8",
      "food_type": "Japanese",
      "Price_for_two": 1463,
      "location": "India Gate",
      "Distance_from_Customer_house": "6.5",
      "offers": 28,
      "alcholol": false,
      "Restaurant_open_time": 3,
      "Restaurant_close_time": 15
    },
    {
      "image": "First",
      "name": "Curry & Co.",
      "rating": "4.4",
      "food_type": "Chinese",
      "Price_for_two": 2381,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "5.0",
      "offers": 15,
      "alcholol": true,
      "Restaurant_open_time": 23,
      "Restaurant_close_time": 11
    },
    {
      "image": "fourth",
      "name": "Golden Thali",
      "rating": "3.4",
      "food_type": "American",
      "Price_for_two": 1476,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "4.0",
      "offers": 15,
      "alcholol": false,
      "Restaurant_open_time": 16,
      "Restaurant_close_time": 4
    },
    {
      "image": "Eight",
      "name": "The Curry House",
      "rating": "4.9",
      "food_type": "Mediterranean",
      "Price_for_two": 1492,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "3.7",
      "offers": 18,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "fourth",
      "name": "Desi Vibes",
      "rating": "4.2",
      "food_type": "Thai",
      "Price_for_two": 2208,
      "location": "Red Fort",
      "Distance_from_Customer_house": "8.2",
      "offers": 32,
      "alcholol": false,
      "Restaurant_open_time": 19,
      "Restaurant_close_time": 7
    },
    {
      "image": "seventh",
      "name": "Mughal Delight",
      "rating": "3.9",
      "food_type": "Mexican",
      "Price_for_two": 2028,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "7.8",
      "offers": 12,
      "alcholol": true,
      "Restaurant_open_time": 10,
      "Restaurant_close_time": 22
    },
    {
      "image": "nine",
      "name": "Biryani Bistro",
      "rating": "4.1",
      "food_type": "American",
      "Price_for_two": 1034,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "1.7",
      "offers": 11,
      "alcholol": true,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "First",
      "name": "Curry & Co.",
      "rating": "4.6",
      "food_type": "American",
      "Price_for_two": 1545,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "5.7",
      "offers": 20,
      "alcholol": false,
      "Restaurant_open_time": 3,
      "Restaurant_close_time": 15
    },
    {
      "image": "First",
      "name": "Spicy Sizzle",
      "rating": "3.5",
      "food_type": "Italian",
      "Price_for_two": 2445,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "9.8",
      "offers": 7,
      "alcholol": false,
      "Restaurant_open_time": 0,
      "Restaurant_close_time": 12
    },
    {
      "image": "third",
      "name": "Kebab King",
      "rating": "4.5",
      "food_type": "American",
      "Price_for_two": 1623,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "3.5",
      "offers": 17,
      "alcholol": false,
      "Restaurant_open_time": 13,
      "Restaurant_close_time": 1
    },
    {
      "image": "fourth",
      "name": "Masala Magic",
      "rating": "3.2",
      "food_type": "Mexican",
      "Price_for_two": 1396,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "8.3",
      "offers": 26,
      "alcholol": false,
      "Restaurant_open_time": 14,
      "Restaurant_close_time": 2
    },
    {
      "image": "fifth",
      "name": "Saffron Spice",
      "rating": "4.9",
      "food_type": "Chinese",
      "Price_for_two": 781,
      "location": "India Gate",
      "Distance_from_Customer_house": "3.3",
      "offers": 18,
      "alcholol": true,
      "Restaurant_open_time": 20,
      "Restaurant_close_time": 8
    },
    {
      "image": "tenth",
      "name": "Desi Vibes",
      "rating": "3.9",
      "food_type": "French",
      "Price_for_two": 1853,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "1.1",
      "offers": 24,
      "alcholol": true,
      "Restaurant_open_time": 13,
      "Restaurant_close_time": 1
    },
    {
      "image": "seventh",
      "name": "Spicy Sizzle",
      "rating": "4.9",
      "food_type": "Indian",
      "Price_for_two": 1629,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "5.9",
      "offers": 22,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "seventh",
      "name": "Tandoor Treat",
      "rating": "3.2",
      "food_type": "American",
      "Price_for_two": 1978,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "2.1",
      "offers": 12,
      "alcholol": false,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "sixth",
      "name": "Saffron Spice",
      "rating": "3.3",
      "food_type": "Thai",
      "Price_for_two": 1410,
      "location": "Red Fort",
      "Distance_from_Customer_house": "8.1",
      "offers": 14,
      "alcholol": true,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "second",
      "name": "Bombay Bites",
      "rating": "5.0",
      "food_type": "Japanese",
      "Price_for_two": 2328,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "10.9",
      "offers": 25,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "fifth",
      "name": "Chaat Corner",
      "rating": "3.2",
      "food_type": "French",
      "Price_for_two": 1484,
      "location": "Red Fort",
      "Distance_from_Customer_house": "5.4",
      "offers": 9,
      "alcholol": true,
      "Restaurant_open_time": 5,
      "Restaurant_close_time": 17
    },
    {
      "image": "First",
      "name": "Mughal Delight",
      "rating": "4.2",
      "food_type": "Chinese",
      "Price_for_two": 831,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "1.8",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 20,
      "Restaurant_close_time": 8
    },
    {
      "image": "seventh",
      "name": "Flavors of India",
      "rating": "4.4",
      "food_type": "Chinese",
      "Price_for_two": 1942,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "10.3",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "third",
      "name": "Saffron Spice",
      "rating": "3.1",
      "food_type": "Mediterranean",
      "Price_for_two": 1474,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "1.2",
      "offers": 29,
      "alcholol": true,
      "Restaurant_open_time": 20,
      "Restaurant_close_time": 8
    },
    {
      "image": "First",
      "name": "The Curry House",
      "rating": "3.5",
      "food_type": "Greek",
      "Price_for_two": 1747,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "6.7",
      "offers": 24,
      "alcholol": false,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "First",
      "name": "Kebab King",
      "rating": "3.9",
      "food_type": "Indian",
      "Price_for_two": 1724,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "3.6",
      "offers": 25,
      "alcholol": false,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "First",
      "name": "Chaat Corner",
      "rating": "4.3",
      "food_type": "Greek",
      "Price_for_two": 697,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "7.5",
      "offers": 19,
      "alcholol": true,
      "Restaurant_open_time": 5,
      "Restaurant_close_time": 17
    },
    {
      "image": "third",
      "name": "Desi Vibes",
      "rating": "3.1",
      "food_type": "French",
      "Price_for_two": 529,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "2.8",
      "offers": 19,
      "alcholol": true,
      "Restaurant_open_time": 8,
      "Restaurant_close_time": 20
    },
    {
      "image": "seventh",
      "name": "Naan Nirvana",
      "rating": "3.7",
      "food_type": "Greek",
      "Price_for_two": 959,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "9.7",
      "offers": 30,
      "alcholol": true,
      "Restaurant_open_time": 12,
      "Restaurant_close_time": 0
    },
    {
      "image": "sixth",
      "name": "Taste of Punjab",
      "rating": "4.7",
      "food_type": "Mexican",
      "Price_for_two": 1629,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "5.7",
      "offers": 23,
      "alcholol": true,
      "Restaurant_open_time": 23,
      "Restaurant_close_time": 11
    },
    {
      "image": "Eight",
      "name": "Desi Vibes",
      "rating": "4.1",
      "food_type": "Indian",
      "Price_for_two": 2377,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "3.4",
      "offers": 30,
      "alcholol": false,
      "Restaurant_open_time": 0,
      "Restaurant_close_time": 12
    },
    {
      "image": "third",
      "name": "Mughal Delight",
      "rating": "4.9",
      "food_type": "Mexican",
      "Price_for_two": 1258,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "5.9",
      "offers": 31,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "seventh",
      "name": "Tandoor Treat",
      "rating": "3.4",
      "food_type": "American",
      "Price_for_two": 1872,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "5.7",
      "offers": 6,
      "alcholol": true,
      "Restaurant_open_time": 4,
      "Restaurant_close_time": 16
    },
    {
      "image": "fourth",
      "name": "Spicy Sizzle",
      "rating": "4.7",
      "food_type": "Greek",
      "Price_for_two": 1500,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "4.6",
      "offers": 18,
      "alcholol": false,
      "Restaurant_open_time": 8,
      "Restaurant_close_time": 20
    },
    {
      "image": "fourth",
      "name": "Spice Garden",
      "rating": "4.0",
      "food_type": "French",
      "Price_for_two": 1347,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "9.1",
      "offers": 26,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "seventh",
      "name": "Royal Rasoi",
      "rating": "4.7",
      "food_type": "Thai",
      "Price_for_two": 1634,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "5.2",
      "offers": 7,
      "alcholol": false,
      "Restaurant_open_time": 18,
      "Restaurant_close_time": 6
    },
    {
      "image": "tenth",
      "name": "Kebab King",
      "rating": "3.4",
      "food_type": "American",
      "Price_for_two": 1523,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "11.0",
      "offers": 23,
      "alcholol": false,
      "Restaurant_open_time": 2,
      "Restaurant_close_time": 14
    },
    {
      "image": "tenth",
      "name": "Masala Magic",
      "rating": "3.2",
      "food_type": "Greek",
      "Price_for_two": 689,
      "location": "India Gate",
      "Distance_from_Customer_house": "5.7",
      "offers": 8,
      "alcholol": true,
      "Restaurant_open_time": 7,
      "Restaurant_close_time": 19
    },
    {
      "image": "second",
      "name": "Mughal Delight",
      "rating": "5.0",
      "food_type": "Indian",
      "Price_for_two": 2127,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "7.9",
      "offers": 33,
      "alcholol": true,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "fourth",
      "name": "The Curry House",
      "rating": "3.9",
      "food_type": "French",
      "Price_for_two": 518,
      "location": "Qutub Minar",
      "Distance_from_Customer_house": "7.6",
      "offers": 7,
      "alcholol": true,
      "Restaurant_open_time": 13,
      "Restaurant_close_time": 1
    },
    {
      "image": "fourth",
      "name": "Mughal Delight",
      "rating": "4.9",
      "food_type": "Greek",
      "Price_for_two": 591,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "3.0",
      "offers": 28,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "fourth",
      "name": "Chaat Corner",
      "rating": "4.7",
      "food_type": "American",
      "Price_for_two": 1546,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "9.3",
      "offers": 32,
      "alcholol": true,
      "Restaurant_open_time": 19,
      "Restaurant_close_time": 7
    },
    {
      "image": "Eight",
      "name": "Tandoori Tales",
      "rating": "3.7",
      "food_type": "Mexican",
      "Price_for_two": 1933,
      "location": "Red Fort",
      "Distance_from_Customer_house": "1.8",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "Eight",
      "name": "Masala Magic",
      "rating": "3.9",
      "food_type": "American",
      "Price_for_two": 1892,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "1.2",
      "offers": 6,
      "alcholol": false,
      "Restaurant_open_time": 9,
      "Restaurant_close_time": 21
    },
    {
      "image": "nine",
      "name": "Bombay Bites",
      "rating": "4.7",
      "food_type": "Chinese",
      "Price_for_two": 1034,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "7.5",
      "offers": 18,
      "alcholol": false,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "second",
      "name": "Indigo Kitchen",
      "rating": "4.2",
      "food_type": "Italian",
      "Price_for_two": 1613,
      "location": "Akshardham Temple",
      "Distance_from_Customer_house": "3.6",
      "offers": 14,
      "alcholol": false,
      "Restaurant_open_time": 23,
      "Restaurant_close_time": 11
    },
    {
      "image": "tenth",
      "name": "Flavors of India",
      "rating": "4.4",
      "food_type": "French",
      "Price_for_two": 564,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "5.2",
      "offers": 22,
      "alcholol": false,
      "Restaurant_open_time": 19,
      "Restaurant_close_time": 7
    },
    {
      "image": "fifth",
      "name": "Flavors of India",
      "rating": "3.6",
      "food_type": "Greek",
      "Price_for_two": 2220,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "4.3",
      "offers": 17,
      "alcholol": true,
      "Restaurant_open_time": 22,
      "Restaurant_close_time": 10
    },
    {
      "image": "fifth",
      "name": "Spice Garden",
      "rating": "4.6",
      "food_type": "Indian",
      "Price_for_two": 2355,
      "location": "Jama Masjid",
      "Distance_from_Customer_house": "3.7",
      "offers": 15,
      "alcholol": false,
      "Restaurant_open_time": 15,
      "Restaurant_close_time": 3
    },
    {
      "image": "seventh",
      "name": "The Curry House",
      "rating": "3.0",
      "food_type": "Thai",
      "Price_for_two": 1070,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "4.9",
      "offers": 30,
      "alcholol": true,
      "Restaurant_open_time": 17,
      "Restaurant_close_time": 5
    },
    {
      "image": "third",
      "name": "Kebab King",
      "rating": "5.0",
      "food_type": "French",
      "Price_for_two": 1702,
      "location": "Purana Qila (Old Fort)",
      "Distance_from_Customer_house": "4.6",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "First",
      "name": "The Curry House",
      "rating": "4.0",
      "food_type": "American",
      "Price_for_two": 1306,
      "location": "Humayun's Tomb",
      "Distance_from_Customer_house": "5.5",
      "offers": 15,
      "alcholol": false,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "second",
      "name": "Spice Garden",
      "rating": "4.5",
      "food_type": "Chinese",
      "Price_for_two": 512,
      "location": "Lodi Gardens",
      "Distance_from_Customer_house": "5.0",
      "offers": 13,
      "alcholol": false,
      "Restaurant_open_time": 7,
      "Restaurant_close_time": 19
    },
    {
      "image": "fourth",
      "name": "Spicy Sizzle",
      "rating": "4.6",
      "food_type": "Greek",
      "Price_for_two": 1337,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "7.4",
      "offers": 24,
      "alcholol": true,
      "Restaurant_open_time": 11,
      "Restaurant_close_time": 23
    },
    {
      "image": "nine",
      "name": "Saffron Spice",
      "rating": "4.2",
      "food_type": "French",
      "Price_for_two": 1014,
      "location": "Lotus Temple",
      "Distance_from_Customer_house": "5.9",
      "offers": 25,
      "alcholol": true,
      "Restaurant_open_time": 1,
      "Restaurant_close_time": 13
    },
    {
      "image": "third",
      "name": "Naan Nirvana",
      "rating": "4.4",
      "food_type": "Italian",
      "Price_for_two": 1151,
      "location": "India Gate",
      "Distance_from_Customer_house": "10.7",
      "offers": 30,
      "alcholol": false,
      "Restaurant_open_time": 3,
      "Restaurant_close_time": 15
    },
    {
      "image": "tenth",
      "name": "Golden Thali",
      "rating": "4.0",
      "food_type": "Mediterranean",
      "Price_for_two": 1903,
      "location": "Hauz Khas Village",
      "Distance_from_Customer_house": "3.7",
      "offers": 24,
      "alcholol": true,
      "Restaurant_open_time": 6,
      "Restaurant_close_time": 18
    }
  ]
function getrestaurant(restaurants){
  const root= document.getElementById('root');
  restaurants.forEach(restaurant=>{

    // create a card
    // 1: Image
    // 2: Card_content
    //      i: Card_header (Name and rating)
    //      ii: Card_footer (Food_type and prices)
    //      iii: Card_location (Restaurant location and distance)


    // create a card div
    const card=document.createElement('div');
    card.classList.add('card');

    // create an Image
    const image=document.createElement('img');
    image.src=`Images/${restaurant.image}.jpeg`;

    // create card-content
    const Card_content=document.createElement('div');
    Card_content.classList.add('card-content');

    // create card header
    const Card_header=document.createElement('div');
    Card_header.classList.add('card-header');

    const h3=document.createElement('h3');
    h3.textContent=restaurant.name;

    const rate=document.createElement('span');
    rate.textContent="Rating: "+restaurant.rating;
    rate.classList.add("rating");

    Card_header.appendChild(h3);
    Card_header.appendChild(rate);

    // create card footer
    const Card_footer=document.createElement('div');
    Card_footer.classList.add('card-footer');

    const food= document.createElement('span');
    food.textContent=restaurant.food_type;

    const price= document.createElement('span');
    price.textContent="₹"+restaurant.Price_for_two;

    Card_footer.appendChild(food);
    Card_footer.appendChild(price);

    // create card location
    const Card_location=document.createElement('div');
    Card_location.classList.add('card-location');
    
    const location= document.createElement('span');
    location.textContent=restaurant.location;

    const distance= document.createElement('span');
    distance.textContent=restaurant.Distance_from_Customer_house+" km";

    Card_location.appendChild(location);
    Card_location.appendChild(distance);

    Card_content.appendChild(Card_header);
    Card_content.appendChild(Card_footer);
    Card_content.appendChild(Card_location);

    card.appendChild(image);
    card.appendChild(Card_content);

    const div1=document.createElement('h4');
    div1.textContent=`Flat ${restaurant.offers} Off`;
    div1.classList.add('off')
    card.appendChild(div1);

    root.appendChild(card);


  });

}

getrestaurant(restaurants);

document.getElementById('Alcohol').addEventListener('click',()=>{
const result=restaurants.filter((obj)=>obj.alcholol);
document.getElementById('root').replaceChildren();
getrestaurant(result);
})

document.getElementById('Rating').addEventListener('click',()=>{
  const result=restaurants.filter((obj)=>obj.rating>=4.5);
  document.getElementById('root').replaceChildren();
  getrestaurant(result);
})

document.getElementById('Offers').addEventListener('click',()=>{
  const result1=[...restaurants];
  result1.sort((a,b)=>b.offers-a.offers)
  document.getElementById('root').replaceChildren();
  getrestaurant(result1);
})

document.getElementById('Open').addEventListener('click',()=>{
  const d = new Date();
  let hour = d.getHours();
  const result=restaurants.filter((obj)=>obj.Restaurant_open_time<=hour&&obj.Restaurant_close_time>hour);
  document.getElementById('root').replaceChildren();
  getrestaurant(result);
})

document.getElementById('Clear').addEventListener('click',()=>{
  document.getElementById('root').replaceChildren();
  getrestaurant(restaurants);
  })

document.getElementById('Filters').addEventListener('click',()=>{
document.getElementById("filterPopup").classList.remove('hidden');
})

document.getElementById('applyFilter').addEventListener('click',()=>{
const element=document.querySelector('input[name="filterOption"]:checked')
const answer=element.value;
let restaurant1 = [...restaurants];

if(answer==="rating"){
  restaurant1.sort((a,b)=>b.rating-a.rating);
}
else if(answer==="highLow"){
  restaurant1.sort((a,b)=>b.Price_for_two-a.Price_for_two);
}
else if(answer==="costLowHigh"){
  restaurant1.sort((a,b)=>a.Price_for_two-b.Price_for_two);
}
else if(answer==="distance"){
  restaurant1.sort((a,b)=>a.Distance_from_Customer_house-b.Distance_from_Customer_house);
}
document.getElementById('root').replaceChildren();
getrestaurant(restaurant1);
})

document.getElementById('closeFilter').addEventListener('click',()=>{
  document.getElementById("filterPopup").classList.add('hidden');
})
